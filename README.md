...
data/
  ...
  final/          <- Deliverable UT datasets (cbcr_main_*.csv). The five files the
                     paper runs on are committed (cleaned, disaggregated, and the
                     three resource-corrected variants); bootstrap draws and
                     sensitivity variants are regenerated by the pipeline.
...
output/           <- see Output layout above
