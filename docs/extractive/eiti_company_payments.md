...
| `1_6_parse_eiti_company_payments.py` | **fallback / cross-check.** Parse the "Part 5 — Company data" sheet of every EITI v2 Summary-Data workbook; join Part-4 GFS; FX-convert via World-Bank rates | `data/raw/resources/eiti_reports/<Country>/*.xlsx*`, `data/raw/API_PA.NUS.FCRF_*.csv` | same `eiti_company_payments_long.csv` |
