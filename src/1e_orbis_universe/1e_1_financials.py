"""
CbCR-universe build — PASS 1 (financials → in-scope groups).
...
(`Consolidation code` C1/C2) `Operating revenue (Turnover)` is **>= EUR 750M**
in any year of the window — i.e. the set of group tops that clear the CbCR
threshold. Pass 2 (a Links_current.txt pass, separate) then expands these to all
subsidiaries via `GUO 50`.

Operating-revenue values in this file are **absolute EUR** (verified: Walmart
US710415188 ~ EUR 563bn), NOT thousands.

Output: data/intermediate/extractive/cbcr_inscope_groups.csv
  bvd_id, peak_consolidated_oprev_eur, n_qualifying_years, qualifying_years
"""
import sys
...
YEARS = set(range(2016, 2024))         # accept 2016..2023 closing years (Jan-2023 closers ~ FY2022)
