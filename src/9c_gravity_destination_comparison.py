# %%
"""
Gravity-sample + destination-based-sales comparison deliverable.

Extends the focused three-scenario deliverable (9_three_scenario_figures.py) so
the headline story also covers (a) the gravity-imputed sample and (b)
destination-based sales, with the comparison figures the report needs:

  PART 1 — Gravity per-scenario figures.
    The same three-scenario figure set as script 9 (Change in taxable profit, Change in tax
    revenue under both rate modes) but built on the GRAVITY-imputed full sample
    (García-Bernardo & Janský 2024 ML imputation — the single disaggregation
    method). Output to output/three_scenarios/gravity/figures/.

  (The former PART 2 — trimmed-mean vs gravity imputation comparison — was removed
  when trimmed-mean disaggregation was retired: there is only one method now.)

  PART 3 — Origin vs destination sales (gravity sample).
    Per scenario, Change in net UT revenue gain by income group under the sales+employees
    formula, ORIGIN sales (unrelated-party revenue booked where recorded) vs
    DESTINATION sales (CFB+digital market key, `sales_employees_destcombined`).
    Plus a low-income country-level figure and table identifying which
    low-income countries still LOSE under destination-based sales.

All figures use the headline spec: average ETR, loss_cit_gain_etr rate mode,
2016–2022 summed, full (imputed) sample. Net metric = revenue_gain_from_ut.

Run order: requires script 5 (gravity datasets, full mode) + script 6 to have
produced summary_country_year_long.csv for the gravity AND trimmed full topics.

Usage: python 9c_gravity_destination_comparison.py
"""
...
def _load_module(stem):
    spec = importlib.util.spec_from_file_location(
        f"_{stem}", str(Path(__file__).resolve().parent / f"{stem}.py")
    )
...
_s8 = _load_module("8_five_scenario_report")
_s9 = _load_module("9_three_scenario_figures")
...
# (key, label, full_topic, full_topic, sample). The gravity-imputed full sample is
# now the single full-mode sample, carried by the plain topics; the third and
# fourth tuple slots are kept (both = the plain topic) so existing unpacking holds.
SCENARIOS = [
    ("baseline", "Resources ignored (baseline)",
...
def net_gain_by_country(topic, formula, etr=ETR, rate=RATE, years=YEARS):
    """Σ revenue_gain_from_ut over years per country for one (formula, etr, rate)."""
    df = load_long(topic)
...
def part1_gravity_scenario_figures():
    ...
    _s9.DATA_BANNER = IMPUTED_BANNER
    _s9.DATA_NOTE = IMPUTATION_NOTE
    summary = _s8.build_summary(YEARS, GRAVITY_SCENARIOS, variant="")
    if summary is None or summary.empty:
        print("  [skip] no gravity summary returned by build_summary")
        return
    by_inc = _s9.build_by_income(summary)
    ...
    # (gravity sample). Reuse script 9's comparison-figure machinery by pointing
    ...
    _s9.SCENARIOS = GRAVITY_SCENARIOS
    _s9.SCEN_SHORT = SCEN_SHORT_GRAVITY
    print("  Three-scenario comparison per formula (gravity)…")
    for fkey, flabel in FORMULA_4FAM:
        _s9.make_formula_scenario_comparison(by_inc, fkey, flabel, figures_dir)
...
def part3_origin_vs_destination():
    ...
    for ax, (key, label, _, grav_topic, _) in zip(axes, SCENARIOS):
        ...
        grouped_income_bars(
            ...
            ORIGIN_DEST_NEXUS,
            ylabel="Change in net UT revenue gain (USD bn)",
            title=label,
        )
    ...
    ax.set_xlabel("Change in net UT revenue gain (USD bn)")
    ax.set_title(
...
def part3c_destination_pct_figures():
    ...
    summ = _s8.build_summary(YEARS, GRAVITY_SCENARIOS_DEST, variant="")
...
def part4_gravity_destination_scenarios(by_inc_dest):
    ...
    destination-based sales (no nexus) on the gravity sample. Reuses script 9's
    make_formula_scenario_comparison on the *_destcombined formula variant, so
    the output mirrors the origin formula_comparison_* figures but on the
    destination key. Writes to output/three_scenarios/gravity/figures/."""
    ...
    for mcol, mlbl in DEST_MEASURES:
        for fkey, flabel in SALES_FAMILIES:
            _s9.make_formula_scenario_comparison(
