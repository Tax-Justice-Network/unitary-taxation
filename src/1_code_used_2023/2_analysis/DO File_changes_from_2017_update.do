...
preserve
keep if cbc=="PROFIT" & grouping=="All Sub-Groups"
keep ultimateparentjurisdiction partnerjurisdiction year value
rename value profits
tempfile profits
save `profits'

restore
keep if cbc=="EMPLOYEES" & grouping=="All Sub-Groups"
keep ultimateparentjurisdiction partnerjurisdiction year value
rename value employees

merge 1:1 ultimateparentjurisdiction partnerjurisdiction year using `profits'
bysort ultimateparentjurisdiction: egen tot_profits = total(profits)
bysort ultimateparentjurisdiction: egen tot_employees = total(employees)
gen share_emp = employees/tot_employees
gen profits_theo = share_emp * tot_profit
gen misalignment = profits - profits_theo

global country "Argentina" 
keep if partnerjurisdiction=="$country"
keep ultimateparentjurisdiction misalignment year
replace misalignment= misalignment/1000000000
reshape wide misalignment, i(ultimateparentjurisdiction) j(year)
rename ultimateparentjurisdiction country
graph bar (asis) misalignment2016 misalignment2017 misalignment2018, over(country, label(angle(45))) ytitle("Misalignment (USD billion)") title("Misalignment of profits in: $country") subtitle("Source: Aggregate CbCR data from OECD") legend(order (1 "2016" 2 "2017" 3 "2018") cols(3))
graph export "C:\Users\AlisonSchultz\Tax Justice Network Ltd\TJN - Shared Documents\Workstreams\Scale of Tax Injustice\State of Tax Justice report\2023 Report\Reasons_for_changes_2017_to_2018\Figures\\${country}\misalignment_over_time_${country}.png", replace

use "${path}/data/raw/estimations/oecd_table1.dta", clear
global country "Argentina"
keep if partnerjurisdiction=="$country" & cbc=="SUBGROUPS_COUNT" & grouping=="All Sub-Groups"
...
rename ultimateparentjurisdiction country
graph bar (asis) value2016 value2017 value2018, over(country, label(angle(45))) ytitle("Number of subsidiaries") title("MNEs' reported subsidiaries in: $country") subtitle("Source: Aggregate CbCR data from OECD") legend(order (1 "2016" 2 "2017" 3 "2018") cols(3))
graph export "C:\Users\AlisonSchultz\Tax Justice Network Ltd\TJN - Shared Documents\Workstreams\Scale of Tax Injustice\State of Tax Justice report\2023 Report\Reasons_for_changes_2017_to_2018\Figures\\${country}\n_companies_over_time_${country}.png", replace
...
rename ultimateparentjurisdiction country
levelsof partnerjurisdiction if partnerjurisdiction=="Argentina", l(jurs) /*Generating this for NLD only for now, as there is an error for countries without any data*/
