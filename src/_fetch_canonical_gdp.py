...
This is the ONE GDP series per country-year used across the repo, consumed by
`1_clean.py` and `1a_destination_based_sales.py` via `config.canonical_gdp_data`.
Run deliberately to refresh (it is a *dated* snapshot, so the pipeline stays
reproducible between refreshes); then point `config.canonical_gdp_data` at the
new dated file.
