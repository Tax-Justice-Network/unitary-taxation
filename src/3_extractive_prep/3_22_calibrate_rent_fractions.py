# %%
"""
3_22 — Calibrate country-specific rent fractions from the WB Resource Rents series.

Calibrate country-specific rent fractions empirically from the WB Resource
Rents indicator: country-level fractions consumed downstream (e.g.
3_24_combined_rents.py), reverse-engineered from WB's own (presumably
country-specific cost-curve) rent series 2016-2021.

For each (iso3, year, commodity):
    implied_rf = WB_rent[iso, c, y] / (volume[iso, c, y] x world_ref_price[c, y])

Sources of volume:
  - oil_gas / coal: EIA International Energy Statistics
  - minerals:       EITI cleaned production panel (HS-coded per commodity)

Then aggregated with the fallback hierarchy:
    1. Country median across 2016-2021 (need >= 2 cells)
    2. Region x IncomeGroup median (fine)
    3. Region median
    4. IncomeGroup median
    5. Global commodity median

Caveats baked into the methodology:
  - WB itself uses a global commodity price x country-modelled cost. Calibrating
...
Pipeline: Extractive prep, stage 3_22 — after 3_21, before 3_23.

Reads:
  data/intermediate/extractive/eiti_production_clean_long.csv — EITI mineral volumes (3_21)
...
Writes:
  data/intermediate/extractive/rent_fractions_calibrated.csv — iso3, commodity, rent_fraction, source_group, n_cells

Usage:
  python 3_22_calibrate_rent_fractions.py
...
def load_classification() -> pd.DataFrame:
    """Read WB country classification (Region + IncomeGroup) from the metadata
    file in data/guides/ (without it the region/income calibration tiers
    silently collapse to the global median)."""
...
def compute_eia_revenue() -> pd.DataFrame:
    ...
    eia = pd.read_csv(RAW / "extractive" / "eia_energy_production.csv")
...
def compute_eiti_minerals_revenue() -> pd.DataFrame:
    ...
    over the EITI-reported HS codes. Built by 3_21_eiti_to_clean_panel.py —
