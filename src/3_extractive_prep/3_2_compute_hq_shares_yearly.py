...
Logic is identical to 3_1_process_orbis_broad.py but the revenue column and
the per-GUO deduplication are applied PER YEAR. That way:
  - Companies with no revenue reported in a given year are excluded that year.
  - GUO dedup picks the biggest-revenue subsidiary in each year separately
    (a different subsidiary may dominate in different years).
  - SOE branch dedup is also applied per year.

Inputs:
  data/raw/resources/orbis_extractives_broad{1,2}.csv
  data/final/company_segment_overrides.csv

Outputs (data/intermediate/extractive/):
  hq_shares_by_commodity_yearly.csv
    — THE pipeline-consumed global HQ shares (read by 1_8, script 4, config.py).
      long format: year, hq_iso3, commodity, revenue_usd, share
  hq_shares_by_commodity_orbis_broad_yearly_long.csv
    — provenance alias of the above (identical long format)
  hq_shares_by_commodity_orbis_broad_yearly_wide.csv
    — wide format: one row per (hq_iso3, commodity), columns share_2016..share_2022
"""
...
OVERRIDES_FILE = os.path.join(RAW, "company_segment_overrides.csv")
...
# ── Helpers (mirrored from 3_1_process_orbis_broad.py) ──
...
# Also write under the name the pipeline actually consumes: 1_8, script 4 and
...
# both here means re-running 3_2 refreshes the file the pipeline reads (previously
# the consumed copy drifted out of date because only the alias was rewritten).
