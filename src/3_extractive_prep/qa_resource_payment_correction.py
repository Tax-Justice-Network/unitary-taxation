"""
Data-quality checks for the resource-payment-correction pipeline (1_6 → 1_7 → 1_8 → 4_).

Checks the upstream artefacts and the three deliverable CbCR datasets:
  - data/intermediate/extractive/eiti_company_payments_long.csv          (1_6)
  - data/intermediate/extractive/eiti_company_hq_map.csv                 (1_7)
  - data/intermediate/extractive/resource_payments_by_hq_source_yearly.csv (1_8)
...
chk("data_source in known set", set(rp.data_source).issubset({"eiti_bilateral", "manual_distributed", "grd_distributed", "rent_proxy"}), str(set(rp.data_source)))
...
chk("unique (source,hq,commodity,year,data_source)", not rp.duplicated(["source_iso3", "hq_iso3", "commodity", "year", "data_source"]).any())
...
keep = keep & ~((pay2.hq_iso3 == pay2.iso3) & (in_u != 1))   # mirror 1_8's domestic-non-MNE drop
pay_matched = pd.to_numeric(pay2.loc[keep, "value_usd"], errors="coerce").clip(lower=0).sum()   # 1_8 floors pre/eq/other at 0
...
chk("PCOL untouched", np.allclose(pd.to_numeric(ci[PCOL], errors="coerce").fillna(0), P_excl))
chk("profit_loss_excl_resource == PCOL - resource_profit_base", np.allclose(
    ce["profit_loss_excl_resource"], P_excl - ce["resource_profit_base_usd"].fillna(0)))
chk("income_tax_..._excl_resource == TCOL - resource_tax_deduction", np.allclose(
    ce["income_tax_paid_on_cash_basis_excl_resource"].fillna(0),
    T_excl - ce["resource_tax_deduction_usd"].fillna(0)))
...
chk("resource_tax_deduction >= post_profit_payments (equity branch can only add)",
    (ce["resource_tax_deduction_usd"].fillna(0) >= ce["post_profit_payments_usd"].fillna(0) - 1e-3).all(),
    f"{(ce['resource_tax_deduction_usd'].fillna(0) < ce['post_profit_payments_usd'].fillna(0) - 1).sum()} rows below")
recomputed_total = ce.groupby(["iso_parent", "year"])["profit_loss_excl_resource"].transform("sum")
chk("total_profit_loss_excl_resource == Σ per (parent,year)", np.allclose(ce["total_profit_loss_excl_resource"], recomputed_total))
...
chk("profit_loss_incl_resource == PCOL + pre_profit_payments", np.allclose(
    ck["profit_loss_incl_resource"], P_incl + ck["pre_profit_payments_usd"].fillna(0)))
chk("income_tax_..._incl_resource == TCOL + pre_profit_payments", np.allclose(
    ck["income_tax_paid_on_cash_basis_incl_resource"].fillna(0),
    T_incl + ck["pre_profit_payments_usd"].fillna(0)))
...
chk("primary alias == cat1 (profit)",
    np.allclose(cef["profit_loss_excl_resource_floored"],
                cef["profit_loss_excl_resource_floored_cat1"]))
chk("primary alias == cat1 (tax)",
    np.allclose(cef["income_tax_paid_on_cash_basis_excl_resource_floored"],
                cef["income_tax_paid_on_cash_basis_excl_resource_floored_cat1"]))
for v in FLEX_VARIANTS:
    ...
    chk(f"profit_loss_excl_resource_floored_{v} == (PCOL - resource_profit_base) - floor_add_on_{v}",
        np.allclose(p_v, profit_excl - addon))
    ...
    chk(f"income_tax_..._excl_resource_floored_{v} == (TCOL - resource_tax_deduction) [floor leaves tax untouched]",
        np.allclose(t_v.fillna(0), tax_excl))
    ...
    chk(f"total_profit_loss_excl_resource_floored_{v} == Σ per (parent,year)",
        np.allclose(cef[f"total_profit_loss_excl_resource_floored_{v}"], recomputed_t))
chk("etr_parent_partner_excl_resource is NaN on all distributed rows",
