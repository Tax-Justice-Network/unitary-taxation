# %%
"""
3_38 — Final cascade: resource payments by HQ × source × commodity × year.
...
Per (source_iso3, hq_iso3, commodity ∈ {oil_gas, coal, minerals, unknown, other}, year):
    ...
    other_payments_usd        — anything classified "other" by 3_31 (kept for completeness)
...
Source-priority cascade per (source, year) cell:
  1. EITI bilateral  — matched company payments → HQ country (3_31 → 3_33).  Used for every
     country that files EITI; unmatched companies are DROPPED (treated as local, per the
     project decision — not redistributed).
  2. Manual          — data/raw/extractive/manual_resource_revenue.csv: a curated country-total table
  ...
  4. Gap-year extrapolation (ALL tiers) — for any covered country (EITI, manual OR GRD)
     ...
     production.
There is no WB rent-proxy tier: every included resource country has a real source
...
Foreign HQ-share cascade for distributed (manual/GRD) totals (audit decision 2 —
no single tier ever fills the WHOLE foreign slice on its own):
  1. EITI bilateral + operator P2G documented payments as ABSOLUTE ANCHORS — each
     covered HQ keeps its own documented amount, never scaled up (a scale-up
     would hand the whole split to whichever operators publish P2G: OMV 27% of Norway,
     Glencore 43.6% of South Africa).
  2. manual_foreign_hq_shares.csv on the residual (renormalised over non-anchored HQs).
  3. CbCR excess-tax weights (_cbcr_excess_tax_weights) — the residual follows the
     (parent → source) cells showing tax above the source's statutory rate, i.e.
     where resource-regime tax is visibly booked and the correction can land.
  4. generic global Orbis hq_shares table — true last resort.
Every output row carries `hq_share_basis` ∈ {domestic, eiti_bilateral, eiti_operator,
manual_foreign, cbcr_excess_tax, generic_global} so the share provenance is auditable.

Extractive prep, stage 3_38 — after 3_37; final cascade; its output is read by
script 4 (resource correction).
...
Reads:
  ...
  data/intermediate/extractive/operator_payments_by_hq_source_yearly.csv — operator P2G anchors (3_34)
  data/intermediate/extractive/extractive_royalty_dataset_yearly.csv   — GRD + WB rents, USD
  data/intermediate/extractive/hq_shares_by_commodity_yearly.csv       — global HQ shares (3_36)
  data/raw/extractive/manual_resource_revenue.csv                      — curated country-total revenues
  data/raw/extractive/manual_foreign_hq_shares.csv                     — hand-curated foreign HQ shares

Writes:
  data/intermediate/extractive/resource_payments_by_hq_source_yearly.csv    — the cascade panel (input to script 4)
  data/intermediate/extractive/resource_payments_by_hq_source_coverage.csv  — coverage/provenance diagnostics
...
MANUAL = DATA_RAW / "extractive" / "manual_resource_revenue.csv"
MANUAL_FHQ = DATA_RAW / "extractive" / "manual_foreign_hq_shares.csv"   # hand-curated foreign HQ splits
PARAMS = DATA_RAW / "extractive" / "resource_country_parameters.csv"   # consolidated control table: include flag + domestic split
...
CASCADE_ORDER = ["eiti_bilateral", "manual", "grd", "eiti_extrapolated"]   # no WB rent-proxy tier
# Priority among HQ-SPLIT bases when the same foreign (hq, commodity) pair was
# split by different bases across covered years — used to collapse those competing
# measurements to one in the gap-year extrapolation (best-evidence first). Lower =
# preferred. `domestic` never competes with a foreign hq, so its rank is inert.
_BASIS_PRIORITY = {"domestic": 0, "eiti_bilateral": 1, "eiti_operator": 2,
                   "manual_foreign": 3, "cbcr_excess_tax": 4, "generic_global": 5}

# Countries where the manual table OVERRIDES EITI bilateral (tax-authority data
# judged more comprehensive than incomplete EITI disclosures) are flagged with
# preferred_source = "manual" in resource_country_parameters.csv, which also
# carries the per-country rationale (preferred_source_note). Read below.
...
def _eiti_hq_share_overrides():
    ...
    For manual_distributed countries (esp. COD/PER/MEX
    where EITI bilateral is dropped via preferred_source = "manual"), the user-curated
    ...
    # value_usd is kept: documented payments act as ABSOLUTE anchors in
    # distribute() (never scaled up to fill
    # the foreign slice), so the $ level matters, not just the relative share.
    return combined[["source_iso3", "year", "commodity", "hq_iso3", "share", "value_usd"]]


def _cbcr_excess_tax_weights():
    """Pooled per-(source, parent) 'excess tax' weights from the cleaned CbCR
    itself: excess = Σ_years max(0, tax − statutory
    CIT × max(profit, 0)) over reported (parent → source) cells. Tax paid above
    the source country's ordinary statutory rate is the bilateral fingerprint of
    resource-regime taxation (petroleum tax, mining surtax), so it locates which
    parents' MNEs actually bear a country's resource take — and, unlike global
    ownership weights, it guarantees the attributed correction lands on CbCR
    cells that exist and can absorb it (e.g. Angola: USA cell ETR 0.80 = Chevron/
    Exxon petroleum tax, while the FRA/GBR cells the P2G anchors point at are
    near-empty). Pooled over all years for stability; all-sector by construction
    (the excess-over-statutory screen keeps ordinary business out); parents with
    excess < $5m dropped as noise. Returns source_iso3, hq_iso3, share."""
    cb_path = (EXT_INT / ".." / ".." / "final" / "cbcr_main.csv").resolve()
    if not cb_path.exists():
        return pd.DataFrame(columns=["source_iso3", "hq_iso3", "share"])
    cb = pd.read_csv(cb_path, usecols=["iso_parent", "iso_partner", "year", "cit",
                                       "profit_loss_before_income_tax_corrected",
                                       "income_tax_paid_on_cash_basis"], low_memory=False)
    cb = cb[(cb.iso_parent.astype(str).str.len() == 3) & (cb.iso_partner.astype(str).str.len() == 3)
            & (cb.iso_parent != cb.iso_partner)]
    cit = pd.to_numeric(cb["cit"], errors="coerce")
    cit = cit.where(cit <= 1.0, cit / 100.0)
    prof = pd.to_numeric(cb["profit_loss_before_income_tax_corrected"], errors="coerce").clip(lower=0)
    tax = pd.to_numeric(cb["income_tax_paid_on_cash_basis"], errors="coerce")
    cb["excess"] = (tax - cit.fillna(0.25) * prof).clip(lower=0)
    g = (cb.dropna(subset=["excess"]).groupby(["iso_partner", "iso_parent"], as_index=False)["excess"].sum()
           .rename(columns={"iso_partner": "source_iso3", "iso_parent": "hq_iso3"}))
    g = g[g["excess"] >= 5e6]
    tot = g.groupby("source_iso3")["excess"].transform("sum")
    g["share"] = g["excess"] / tot
    return g[["source_iso3", "hq_iso3", "share"]]


def _manual_foreign_hq_shares():
    """Hand-curated foreign-HQ splits (data/raw/extractive/manual_foreign_hq_shares.csv):
...
def distribute(source_iso3, commodity, year, amount_pre, amount_post, amount_eq, domestic_share, hq_tbl,
               eiti_share_override=None, manual_fhq=None, excess_tbl=None):
    ...
    Foreign-share cascade (audit decision 2):
      1. `eiti_share_override` — EITI bilateral + operator P2G documented payments,
         used as ABSOLUTE ANCHORS: each covered HQ keeps its own documented amount
         (converted to a weight via the cell take), and is NEVER scaled up to fill
         the whole foreign slice. A scale-up would hand the entire
         foreign split to whichever operators happen to publish a P2G report
         (OMV 27% of Norway, Glencore 43.6% of South Africa) — the operator-
         coverage skew. Anchors are collectively capped at the foreign slice.
      2. `manual_fhq` — hand-curated foreign HQ shares (manual_foreign_hq_shares.csv),
         applied to the RESIDUAL after anchors, renormalised over the HQs that do
         not already have an anchor (evidence outranks inference, so the curated
         splits sit above the CbCR-derived weights).
      3. `excess_tbl` — CbCR excess-tax weights (_cbcr_excess_tax_weights): the
         residual follows the (parent → source) cells that actually show tax paid
         above the source's statutory rate, i.e. where resource-regime tax is
         visibly booked and the correction can land.
      4. generic global Orbis `hq_tbl` — true last resort.

    Each returned dict carries `hq_share_basis` ∈ {domestic, eiti_operator,
    manual_foreign, cbcr_excess_tax, generic_global} for auditability."""
    rows = []
    ...
    if fw <= 0:
        return rows
    cell_take = amount_pre + amount_post + amount_eq

    def _emit(hq, w, basis):
        rows.append({"hq_iso3": hq, "pre_profit_payments_usd": amount_pre * w,
                     "post_profit_payments_usd": amount_post * w, "equity_income_usd": amount_eq * w,
                     "other_payments_usd": 0.0, "hq_share_basis": basis})

    # 1. absolute anchors from documented payments
    # ANCHOR_EXCLUDE (ARE resolution): Gulf-concession P2G figures are
    # dominated by IN-KIND production entitlements remitted on the state's behalf
    # (TotalEnergies reports $6-25bn/yr "paid" to Abu Dhabi), so they wildly
    # overstate the operator's own fiscal share - anchoring on them would hand
    # FRA/GBR the whole ARE foreign slice while the actual taxpayers (INPEX/CNPC/
    # Exxon, cells at 0.76-0.87 ETR) get nothing. Skip anchors for these sources; the
    # manual concession-equity shares (+ excess-tax residual) take over.
    ANCHOR_EXCLUDE = {"ARE"}
    anchored = {}
    if (source_iso3 not in ANCHOR_EXCLUDE
            and eiti_share_override is not None and len(eiti_share_override) and cell_take > 0):
        ...
        if len(ev):
            w_anchor = (ev.set_index("hq_iso3")["value_usd"] / cell_take).clip(lower=0.0)
            scale = min(1.0, fw / w_anchor.sum()) if w_anchor.sum() > 0 else 0.0
            for hq, wi in (w_anchor * scale).items():
                if wi > 0:
                    _emit(hq, wi, "eiti_operator")
                    anchored[hq] = wi
    resid = fw - sum(anchored.values())
    if resid <= 1e-12:
        return rows

    # 2. manual shares on the residual (non-anchored HQs only)
    if manual_fhq is not None and len(manual_fhq):
        mv = manual_fhq[
            (manual_fhq["source_iso3"] == source_iso3)
            & (manual_fhq["year"] == year)
            & (manual_fhq["commodity"] == commodity)
            & (manual_fhq["hq_iso3"] != source_iso3)
            & (~manual_fhq["hq_iso3"].isin(anchored))
        ]
        if len(mv) and mv["share"].sum() > 0:
            s = mv.set_index("hq_iso3")["share"]
            for hq, wi in (s / s.sum() * resid).items():
                _emit(hq, wi, "manual_foreign")
            return rows

    # 3. CbCR excess-tax weights on the residual
    if excess_tbl is not None and len(excess_tbl):
        xv = excess_tbl[(excess_tbl["source_iso3"] == source_iso3)
                        & (excess_tbl["hq_iso3"] != source_iso3)
                        & (~excess_tbl["hq_iso3"].isin(anchored))]
        if len(xv) and xv["share"].sum() > 0:
            s = xv.set_index("hq_iso3")["share"]
            for hq, wi in (s / s.sum() * resid).items():
                _emit(hq, wi, "cbcr_excess_tax")
            return rows

    # 4. generic global Orbis weights
    foreign = hq_tbl[(hq_tbl.year == year) & (hq_tbl.commodity == commodity)
                     & (hq_tbl.hq_iso3 != source_iso3) & (~hq_tbl.hq_iso3.isin(anchored))]
    fsum = foreign["share"].sum()
    if fsum > 0:
        for _, r in foreign.iterrows():
            _emit(r["hq_iso3"], (r["share"] / fsum) * resid, "generic_global")
    else:
        # no foreign-share info → put the remainder on the source country too
        rows.append({"hq_iso3": source_iso3, "pre_profit_payments_usd": amount_pre * resid,
                     "post_profit_payments_usd": amount_post * resid, "equity_income_usd": amount_eq * resid,
                     "other_payments_usd": 0.0, "hq_share_basis": "domestic"})
    return rows


# %% MARK: 4. EITI bilateral tier
# ── 1. EITI bilateral ────────────────────────────────────────────────────────
def eiti_bilateral():
    ...
    n_all = len(pay)
    # GFS 1153* = "revenues from sale of the state's share of production": the payer
    # is a CUSTOMER of the state marketer (SOMO crude buyers - UNIPEC/Indian Oil/
    # Valero/TOTSA trading arms), not a producer paying fiscal obligations ->
    # attribute DOMESTIC, never to the buyer's HQ (materially Iraq, $390bn
    # in-window). These rows keep an hq for the panel even
    # when the buyer was unmatched, and are exempt from the domestic-non-MNE drop
    # below (they are state revenue, not an operator's payment).
    def _state_sales_mask(d):
        # 1153* anywhere; for IRQ additionally the whole equity bucket - Iraq has no
        # foreign-equity dividends, so every equity-classified row is a SOMO crude
        # sale whose gfs code is missing in some report vintages (buyers: Chennai
        # Petroleum, UNIPEC, Reliance, Sinochem...).
        m = d["gfs_code"].astype(str).str.startswith("1153")
        return m | ((d["iso3"] == "IRQ") & (d["revenue_type"] == "equity"))
    sales_mask = _state_sales_mask(pay)
    pay.loc[sales_mask, "hq_iso3"] = pay.loc[sales_mask, "iso3"]
    ...
    drop_domestic_nonmne = (pay["hq_iso3"] == pay["iso3"]) & (in_u != 1) & ~sales_mask
...
# %% MARK: 5. Manual and GRD tiers
# ── 2./3./4. cascade fill for non-EITI countries ─────────────────────────────
def manual_rows(year_set, hq_tbl, manual_fhq=None, eiti_overrides=None, excess_tbl=None):
    ...
    # Applies to ALL manual entries (not just COD/PER/MEX, whose manual totals
    # override EITI) —
    ...
    for _, r in m.iterrows():
        ...
        for d in distribute(r["source_iso3"], comm, int(r["year"]), pre, post, eq, r["domestic_share"], hq_tbl,
                            eiti_share_override=override_df, manual_fhq=manual_fhq, excess_tbl=excess_tbl):
...
def build_eiti_splits(eiti_df):
    ...
    plus the folder Summary-Data splits (3_13). Returns (by_year, by_country):
...
def grd_rows(non_eiti_with_manual, eiti_countries, year_set, hq_tbl, ds_tbl=None,
             eiti_by_year=None, eiti_by_country=None, eiti_share_override=None, manual_fhq=None, excess_tbl=None):
    ds_tbl = ds_tbl or {}
    ...
    for _, row in rd.iterrows():
        ...
        for comm, cf in csplit.items():
            ...
            for d in distribute(iso, comm, int(row["year"]), pre * cf, post * cf, eq * cf, dom, hq_tbl,
                                eiti_share_override=eiti_share_override, manual_fhq=manual_fhq, excess_tbl=excess_tbl):
...
def extrapolate_gap_years(real_df, covered_pairs, needed_partners, prof, oow_base=None):
    ...
    Principle: a missing year is almost never "production stopped" —
    ...
    `oow_base`: OUT-OF-WINDOW EITI rows (2010–2015, 2023+).
    ...
    structure base; the PROFIT scaler is anchored on in-window covered years only
    (averaging CbCR profit over ALL covered years would let out-of-window
    years enter that mean as zeros — deflating it and inflating the scaler). A country
    ...
    Note on the output schema (revised 2026-07-26): the covered-year take is
    pooled per (hq_iso3, commodity), NOT per hq_share_basis, so the (source, hq,
    commodity, year) key is UNIQUE. Earlier this pooled per basis, which
    cross-summed a foreign pair that used DIFFERENT competing bases in different
    covered years (e.g. AZE→GBR oil_gas: eiti_operator in 2017/2018/2022 but
    eiti_bilateral in 2019–2021) — the payment audit's DUPLICATE_SOURCE_HQ_
    COMMODITY_YEAR flag. Those bases are competing measurements of the same
    payments, not additive slices, so they are now collapsed to one pooled take
    (Σ buckets ÷ n_cov) tagged with the highest-priority basis (_BASIS_PRIORITY).
    Domestic vs foreign remain separate (different hq_iso3) and additive as
    before; no (source, year) cell carries more than one real tier."""
    ...
    for src in sorted(base_df["source_iso3"].dropna().unique()):
        ...
        if not gap_years or n_cov == 0:
            continue
        # Pool the covered-year take per (hq_iso3, commodity) — deliberately NOT
        # per hq_share_basis. A given foreign (hq, commodity) pair can be split by
        # DIFFERENT bases in different covered years (e.g. AZE→GBR oil_gas: an
        # eiti_operator basis in 2017/2018/2022 but an eiti_bilateral basis in
        # 2019–2021). Those bases are COMPETING measurements of the same payments,
        # not additive slices, so grouping by basis and emitting one row each would
        # cross-sum them into the gap year — the double-count the payment audit
        # flagged for AZE/DNK. Grouping by (hq, commodity) only pools them into one
        # take (Σ buckets over covered years ÷ n_cov), exactly as the main cascade
        # collapses competing bases by priority (§drop_duplicates above) and as
        # script 4 group-sums the additive domestic/foreign slices within a year.
        bcol = sub.get("hq_share_basis", pd.Series("unknown", index=sub.index)).rename("hq_share_basis")
        sub = sub.assign(hq_share_basis=bcol)
        agg = sub.groupby(["hq_iso3", "commodity"])[BUCKETS].sum() / n_cov
        # representative basis per (hq, commodity) = the highest-priority basis seen
        # across the covered years, for auditability of the emitted row.
        rep_basis = (sub.assign(_p=sub["hq_share_basis"].map(_BASIS_PRIORITY).fillna(9))
                        .sort_values("_p")
                        .drop_duplicates(["hq_iso3", "commodity"], keep="first")
                        .set_index(["hq_iso3", "commodity"])["hq_share_basis"].to_dict())
        # Profit anchor: IN-WINDOW covered years only. Out-of-window years have no
        # CbCR profit, so including them would pull the
        # mean toward zero and inflate the scaler.
        ...
        for gy in gap_years:
            ...
            for (hq, comm), vals in agg.iterrows():
                ...
                rec.update({"source_iso3": src, "hq_iso3": hq, "commodity": comm,
                            ...
                            "hq_share_basis": rep_basis.get((hq, comm), "extrapolated")})
                out.append(rec)
            covered.add((src, gy))
...
def main():
    ...
    # countries whose manual entry replaces (not augments) their EITI bilateral data
    manual_overrides_eiti = set(
        params.loc[params["preferred_source"].fillna("").astype(str).str.lower() == "manual", "iso3"])
    ...
    excess_tbl = _cbcr_excess_tax_weights()
    print(f"  CbCR excess-tax weight table: {excess_tbl.source_iso3.nunique() if len(excess_tbl) else 0} source countries")
    ...
    # guess for a missing 2016, EITI 2023 for a missing 2022.
    ...
    # preferred_source == "manual" (resource_country_parameters.csv): drop EITI
    # rows for those countries so the manual entry below replaces (rather than
    # augments) them.
    ...
    # Per-(source, year) cascade: rather than a per-country cascade
    ...
    # GRD fills where neither EITI nor manual have data; the gap-year
    # extrapolation fills remaining covered-country gaps.
    #
    # EITI→GRD scale-up:
    ...
    man_df, _ = manual_rows(year_set, hq_tbl, manual_fhq=manual_fhq, eiti_overrides=ovr, excess_tbl=excess_tbl)
    if len(man_df):
    ...
    grd_df, _ = grd_rows(set(), set(), year_set, hq_tbl, ds_tbl=ds_tbl,
                         ...
                         eiti_share_override=ovr, manual_fhq=manual_fhq, excess_tbl=excess_tbl)
    if len(grd_df):
