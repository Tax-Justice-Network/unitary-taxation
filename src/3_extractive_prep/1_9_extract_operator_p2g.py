...
Consumes (from data/raw/resources/country sources/):
  - general/Report-on-payments-to-governments-*.pdf      → Eni (2017-2024, 7 years)
  - general/ddr{2016,2017,2018}-en*.pdf                  → TotalEnergies Registration Documents (Ch. 9.3)
  - general/2019_total_*.pdf, 2020-universal-*.pdf       → TotalEnergies URDs (Ch. 9.3)
  - general/TotalEnergies_URD_2022_EN.pdf                → TotalEnergies URD 2022
  - general/totalenergies_universal-registration-*.pdf   → TotalEnergies URDs 2023-2025
  - general/bp-report-on-payments-to-governments-*.pdf   → BP (2016-2024)
  - general/xom*.htm                                     → ExxonMobil 10-Ks (segment data, no detailed P2G)
  - COD/GLEN*.pdf                                        → Glencore P2G 2018-2025
  - MOZ/Reports on Payments to Governments *.pdf        → Vale P2G 2022-2024
  - MOZ/{Tax Transparency Report,Vale_TTR}*.pdf         → Vale TTR 2019-2022
  - MOZ/Sasol_TAX*.pdf, SASOL*.pdf, sasol_*.pdf         → Sasol Integrated/Tax Reports
  - YEM/galp_payments.pdf, MOZ/Galp*.pdf                → Galp P2G / Annual Reports
...
SOURCES = RAW / "resources" / "country sources"
...
def main():
    ...
    `{operator_slug}_{report_type}_{year}.pdf` naming (set up 2026-05-17).
    Country subfolders no longer hold operator P2G/tax/AR documents — those
    live directly under `data/raw/resources/country sources/`."""
    rows = []
    ...
    for yr, p in each_year("total_urd_"):
        r = parse_total_urd(p)
        rows.extend(r)
    ...
    for yr, p in each_year("glencore_p2g_"):
    ...
    for yr, p in each_year("vale_ttr_", years=range(2019, 2025)):
        r = parse_vale_ttr(p, yr)
        rows.extend(r)
        print(f"  Vale TTR {yr}: {len(r)} country rows from {p.name}")

    # ── Vale Swiss-format P2G (2022-2024) ──
    for yr, p in each_year("vale_p2g_", years=range(2022, 2025)):
    ...
    for yr, p in each_year("sasol_tax_", years=range(2024, 2026)):
        r = parse_sasol_tax_report(p, yr)
        rows.extend(r)
        print(f"  Sasol {yr}: {len(r)} country rows from {p.name}")

    # ── Saudi Aramco AR (SAU domestic) ──
    for yr, p in each_year("aramco_ar_", years=range(2019, 2026)):
    ...
    for yr, p in each_year("antofagasta_p2g_", years=range(2016, 2025)):
    ...
    for yr, p in each_year("riotinto_taxes_", years=[2016] + list(range(2018, 2025))):
    ...
    for yr, p in each_year("newmont_tax_", years=range(2021, 2025)):
    ...
    for yr, p in each_year("firstquantum_tax_", years=range(2020, 2025)):
