"""
Build per-(source_country, commodity, year) HQ-share table for the
resource_factor refinement in the SOTJ profit-shifting pipeline.
...
`share` sums to 1.0 within each (source_iso3, commodity, year) group.

Methodology
-----------
For each (entity_country=S, guo_country=H, commodity=C, year=Y):
  1. Sum 'Operating revenue Y' across all entities matching those criteria.
  2. Normalise within each (S, C, Y) group.

Coverage handling
-----------------
- Entities without revenue in year Y fall back to
    peak_operating_revenue_th_usd / n_years_with_revenue
  (per-bvd_id), so entities that lacked Y data still contribute a proxy.
- Drop entities whose HQ (guo_country, with entity_country fallback) is missing.
- Drop tiny rows (proxy revenue < $100k).

Small-market fallback
---------------------
If a (source, commodity, year) group has < 3 distinct entities OR aggregate
revenue < $10M USD, fall back to the GLOBAL HQ share from
`hq_shares_by_commodity_yearly.csv`. Flag those rows fallback=True.

Reuses the NACE→commodity mapping and segment-overrides logic from
`3_2_compute_hq_shares_yearly.py` (oil_gas | minerals | coal).
"""
...
# ── Helpers (mirrored from 3_2_compute_hq_shares_yearly.py) ──
