# %%
"""
3_32 — Build the Orbis extractive-entity universe.

Build the full Orbis extractive-entity universe used by 3_33 to attach an HQ
country to every EITI company. Matching only against the ~6k deduplicated
top-level companies is not enough: the raw Orbis pull contains the full
extractive universe (~600k entities, each carrying its GUO bvd-id / name / type /
country directly), i.e. it already includes local operating subsidiaries
("Total E&P Angola", "Shell Nigeria", "Kibali Goldmines SA", …). Matching an
EITI company string against that universe (blocked by the country it pays in)
and reading off the matched entity's GUO country is the proper way to get the HQ.
Output flag in_cbcr_universe = (n_subsidiaries >= 2) OR (peak revenue >= €750M) —
a multi-entity extractive group above the CbCR turnover threshold; revenue
missing does not disqualify.

Extractive prep, stage 3_32 — after 3_31, before 3_33.

Reads (all in data/raw/orbis/):
  extractives_guo_orbis*.csv / .xlsx  — Company name, BvD ID, GUO BvD/Name/Type/Country (entity→GUO)
  extractives_static_orbis*.csv       — BvD ID, Country ISO, NACE core, No of subsidiaries
  extractives_financials_orbis*.csv   — BvD ID, Operating revenue (Turnover) th USD 2016..2025
                                        (batches 33/35/36/38/39/42 missing → ~15% of entities have no revenue)

Writes:
  data/intermediate/extractive/orbis_entity_universe.csv  — one row per Orbis bvd_id: name,
    entity_country, nace_core, n_subsidiaries, guo_bvd_id, guo_name, guo_type, guo_country,
    peak_operating_revenue_usd, in_cbcr_universe

Usage:
  python 3_32_build_orbis_entity_universe.py
