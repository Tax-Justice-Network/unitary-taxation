# %%
"""
3_39 — Per-country resource info dossiers.

For every jurisdiction that receives a resource correction, write a
human-readable `.txt` dossier — a country reference (for when people ask about a
specific country) and a QA audit of the applied correction in one. Each sheet
carries: CORRECTION SOURCE(S) (cascade tier(s) eiti / manual / grd / rent_proxy
+ years); RESOURCES & RENT-CAPTURE REGIME (commodities; statutory CIT baseline;
effective resource rate r; how rent is captured — royalties = pre-profit,
resource income tax / CIT = post-profit, state/NOC dividends = equity — with the
split fractions and domestic_share; regime notes); RATES (c, r, per-commodity
profit-tax rates); SOURCES (per-commodity + combined-rate + manual-revenue
source notes, provenance doc pointers); REPORTED & CORRECTION BY YEAR (profit /
tax / observed ETR vs stripped vs residual); DOMESTIC vs FOREIGN pooled (foreign
non-resource ETR after correction); FLAGS (negative residual, over-strip, etc.).
Read-only on all inputs.

Extractive prep — per-country resource dossiers; runs AFTER script 4 (reads its
corrected output).

Reads:
  data/final/cbcr_main_excl_resource.csv                                — resource-corrected dataset (script 4)
  data/raw/extractive/resource_profit_tax_rate.csv                      — per-commodity profit-tax rates
  data/raw/extractive/effective_resource_cit_rates.csv                  — effective resource CIT rates
  data/raw/extractive/manual_resource_revenue.csv                       — curated country-total revenues
  data/intermediate/extractive/resource_payments_by_hq_source_yearly.csv — the cascade panel (3_38)

Writes:
  output/extractive/country_resource_info/<ISO3>.txt   — per-country dossier
  output/extractive/country_resource_info/_SUMMARY.csv — cross-country summary table
  output/extractive/country_resource_info/_SUMMARY.txt — cross-country summary text

Usage:
  python 3_39_country_resource_info.py
...
OUT = RAW.parent.parent / "output" / "extractive" / "country_resource_info"
B = 1e9
DOCS = RAW.parent.parent / "docs" / "extractive"   # provenance docs the sheets cite
_URL_RE = re.compile(r"https?://[^\s)\]`|]+")


# %% MARK: 2. Config and helpers
...
# Mirrors 3_38_resource_payments_by_hq_source.py DEFAULT_SPLIT / DEFAULT_DOMESTIC.
...
# country's resource estimate (the cascade in 3_38_resource_payments_by_hq_source.py).
SOURCE_NAME = {
...
def main():
    ...
    rt = pd.read_csv(RAW / "extractive" / "resource_profit_tax_rate.csv", comment="#")
    ecr = pd.read_csv(RAW / "extractive" / "effective_resource_cit_rates.csv", comment="#").dropna(subset=["iso3"])
    ecr = ecr[ecr.iso3.str.len() == 3].set_index("iso3")
    mrr = pd.read_csv(RAW / "extractive" / "manual_resource_revenue.csv", comment="#")
    ...
    _par = pd.read_csv(RAW / "extractive" / "resource_country_parameters.csv")
    ...
    for iso in corr_iso:
        ...
        if "eiti" in _dm:
            L.append(f"For {_nm}, the figures come from resources/eiti_reports, so the attribution is bilateral: each")
            ...
            if "extrapolated" in _dm:
                ...
                L.append("in years the resources/eiti_reports do not yet cover).")
            else:
