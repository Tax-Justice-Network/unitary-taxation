...
Sources joined per (iso3, year, category):
  - WB         data/raw/resources/wb_resource_rents.csv             (commodity rents in USD)
  - EIA        data/raw/resources/eia_energy_production.csv          (production volumes)
  - EITI       data/final/eiti_rents_estimated_yearly.csv  (rent estimate computed
                                                            from EITI production)
...
Output: data/final/rents_combined_yearly.csv
    iso3, year, category,
    rent_wb_usd, rent_eia_usd, rent_eiti_usd,
    rent_best_usd, rent_best_source,
    eia_revenue_usd, eia_unit_price, eia_volume_units,
    eiti_n_commodities, eiti_commodity_list,
    wb_published, wb_carry_status

For 2021-2022, also writes a wide pivoted view:
    data/final/rents_combined_2021_2022_wide.csv
"""
...
def load_calibrated_rent_fractions() -> pd.DataFrame:
    """Read 2_8a output. Returns DataFrame with iso3, category, rent_fraction,
    ...
    if not fp.exists():
        print("  [warn] rent_fractions_calibrated.csv missing; falling back "
              "to flat defaults. Run 2_8a_calibrate_rent_fractions.py first.")
...
def load_bgs_scaling_factors() -> dict:
    """Read 2_8b output. Returns dict mapping (iso3, category) -> (scale,
    ...
    if not fp.exists():
        print("  [warn] bgs_scaling_factors.csv missing; BGS rents will not "
              "be scaled to WB level. Run 2_8b_calibrate_bgs_scaling.py first.")
        return {}
    df = pd.read_csv(fp)
...
def load_eia_rents(calib_lookup: dict) -> pd.DataFrame:
    ...
    eia_raw = pd.read_csv(RAW / "resources" / "eia_energy_production.csv")
...
def load_eiti_rents(calib_lookup: dict) -> pd.DataFrame:
    ...
    Uses the `gross_revenue_usd` blend built by 2_7_eiti_to_clean_panel.py —
    ...
    multiplied by the calibrated country-level rent fraction from 2_8a.
    """
...
def load_bgs_rents(calib_lookup: dict, bgs_scale_lookup: dict) -> pd.DataFrame:
    ...
    Reads data/raw/resources/bgs_mineral_production.csv (produced by
    1_7_pull_bgs_minerals.py) and pipes mineral-commodity volumes through the
    ...
    # mineral_rents (which lumps every mineral). See 2_8b for calibration.
