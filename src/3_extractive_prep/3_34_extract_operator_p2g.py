# %%
"""
3_34 — Extract operator Payments-to-Governments disclosures.

Extract operator-side Payments-to-Governments data from major IOC/NOC
disclosures — PDF documents named {HQISO3}_{Operator}_{DocType}_{year}.{ext}
(e.g. ITA_Eni_P2G_2019.pdf, GBR_BP_P2G_2020.pdf, FRA_TotalEnergies_URD_2022.pdf,
CHE_Glencore_P2G_2018.pdf, SAU_Aramco_AR_2019.pdf, …); see the folder's
_rename_manifest_2026-07-21.csv for the mapping from original download names.
Each payment category maps to a bucket ∈ {pre_profit, post_profit, equity, fee,
other}: royalties / licence fees / production entitlements / bonuses →
pre_profit; taxes (income tax) → post_profit; dividends → equity;
infrastructure improvements / fees → fee (small, excluded from main aggregation).
Downstream consumer 3_38 merges this operator data in so manual_distributed
totals get HQ-shares anchored on operator-paid data wherever it exists (not just
generic Orbis).

Extractive prep, stage 3_34 — after 3_33, before 3_35.

Reads:
  data/raw/extractive/resource_profits_manual_sources/company/*  — operator P2G PDF disclosures
...
Usage:
  python 3_34_extract_operator_p2g.py
...
SOURCES = RAW / "extractive" / "resource_profits_manual_sources" / "company"
OUT = EXT_INT / "operator_payments_by_hq_source_yearly.csv"

# %% MARK: 2. Country lookups
...
# %% MARK: 3. Table extraction helpers
...
def main():
    ...
    `{HQISO3}_{Operator}_{DocType}_{year}.pdf` naming.
    All operator P2G/tax/AR documents live directly under
    `data/raw/extractive/resource_profits_manual_sources/company/`."""
    rows = []
    flat = SOURCES   # files live directly here
    ...
    # (No Aramco parser: its only rows would be SAU→SAU, and the write loop
    # below drops domestic source_iso3 == hq_iso3 rows by design — the SAU
    # domestic flow enters via the manual/GRD tiers instead.)

    # ── EU/UK statutory P2G operators — all share
    ...
    # OMV's Austrian statutory P2G report is published "in EUR 1,000", NOT USD
    # millions — a 1e6 factor would inflate every OMV figure ~1000× (Norway
    # 2022 would come out at $2.17tn) and produce an AUT operator-skew
    # in NOR/KAZ/YEM/MYS/ROU; hence currency_factor=1e3.
    for yr, p in each_year("AUT_OMV_P2G_", years=[2018, 2021, 2022, 2023, 2024]):
        r = parse_eu_p2g(p, yr, "AUT", "OMV",
                         currency_factor=1e3, fx_to_usd=1.10,
