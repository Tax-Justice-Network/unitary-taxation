"""
EITI company payments-to-government, brought to a (HQ country × source country)
level using the Orbis HQ map.
...
Outputs: data/intermediate/extractive/eiti_payments_by_hq_source_yearly.csv
           one row per (source iso3, HQ iso3, commodity, fiscal year, revenue type)
           with the summed USD value, row count, and an "assume-domestic" variant
           of the HQ for rows where the company could not be matched to Orbis.
         data/intermediate/extractive/eiti_payments_by_hq_source_summary.csv
           coarser: (source iso3, HQ iso3, fiscal year) totals.

`hq_iso3`            = matched HQ iso3, or "UNMATCHED" / "UNKNOWN_HQ" when the EITI
                       company could not be tied to an Orbis company (or it was but
                       its HQ country is unknown).
`hq_iso3_domestic`   = same, but UNMATCHED/UNKNOWN is replaced by the source country
                       (i.e. treat unmatched companies as domestic operators).
"""
import csv
import sys
...
def main():
    ...
    with open(PAY, encoding="utf-8") as f:
        for r in csv.DictReader(f):
            ...
            # fy_end_year). The earlier fy_end_year lookup returned empty for
            # every row → all aggregations collapsed onto fy_end_year=NaN,
            # making COD/MDG/etc. invisible in the yearly panel. Fixed
            # 2026-05-15.
