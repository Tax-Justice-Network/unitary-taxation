# %%
"""
1 — Clean and merge the OECD CbCR panel and its auxiliary data.

Imports and cleans the aggregate country-by-country reports (CbCR) published by
the OECD, applies the dividend double-counting corrections (Garcia-Bernardo,
Jansky & Zucman), and merges the auxiliary data the later scripts need — CIT
rates, wages, GDP/population, regions and World Bank income groups (incl. the
tax-haven flag). Computes the canonical reported-profit ETR family via
_etr_construction.compute_partner_year_etrs. Stateless (STLS) and other
unlocatable partner rows are dropped rather than kept.

Section outline (numbers match the cell headings below):
1. Setup (imports, technical helpers).
2. Import and clean CbCR data — 2.1 filter to analysis years and drop unusable
   partner rows; 2.2 dividend double-counting corrections; 2.3 partner-year ETRs.
3. Import and clean the auxiliary data, one source per subsection —
   3.1 country-year sample grid; 3.2 corporate income tax rates; 3.3 GDP and
   population; 3.4 wages (ILO + imputations); 3.5 regions and WB income groups.
4. Merge everything, save the two output datasets, run final checks.

Pipeline step 1 — first step; before 1a.

Reads:
  {data_raw}cbcr/oecd_cbcr_tableI_2026-02.csv               — OECD CbCR Table I (main input; gitignored)
  {data_raw}tax_rates/oecd_cit_rates_2026-02.csv            — OECD statutory CIT rates
  {data_raw}tax_rates/… (Tax Foundation, .xlsx)             — Tax Foundation CIT rates (fills OECD gaps)
  {data_raw}macro_variables/manual_imputation_values.csv    — hand-curated GDP/pop/CIT/wage overrides + sources
  {data_raw}macro_variables/wb_gdp_population_2026-02.csv    — World Bank GDP & population (DataBank export)
  {data_raw}macro_variables/wb_gdp_current_usd_2026-07.csv   — canonical WB GDP snapshot (override, applied post-grid)
  {data_raw}macro_variables/ilo_wages_ear4mth_2025-08.csv    — ILO monthly wages
  {data_raw}country_info/… (unilateral_cross)               — regions (region_tjn) and helper flags
  {data_raw}country_info/wb_income_groups_2026-07.csv        — pinned WB income-group snapshot (auto-created from API once, then reused)

Writes:
  {data_final}/cbcr_main.csv                                — all reporting groupings + merged auxiliary columns
  {data_final}/cbcr_main_allsubgroupsonly.csv               — "Total (All sub-groups)" panel only; the working sample read by 1a and 2
  {data_raw}country_info/wb_income_groups_2026-07.csv        — written on first run only (pinned snapshot from the WB API)

Usage:
  python 1_clean.py
  Before running: conda activate unitary_taxation; set first_year / n_years and
  the raw-data paths in config.py. Only when updating to a new CbCR vintage,
  consult the OECD disclaimer and country notes for reporting anomalies:
    https://www.oecd.org/tax/tax-policy/anonymised-and-aggregated-cbcr-statistics-disclaimer.pdf
    https://www.oecd.org/tax/tax-policy/COUNTRY-cbcr-country-specific-analysis.pdf

Author: Alison Schultz (based on Javier Garcia-Bernardo's work).
Created: 2023-08-04.  Last updated: 2026-07-25.
"""

# %% MARK: 1. Setup (technical helpers)
# 0. Setup — imports and technical helpers only (ISO lookup, duplicate/
#    missing-value checks); functions with methodological content live in
#    their concept sections below
...
# cannot print the non-ASCII this script emits (accented country names,
# dashes in messages) and would crash with UnicodeEncodeError. Force UTF-8
# output; harmless elsewhere.
...
def get_iso3_from_name(name):
    """Country name → ISO3, without depending on the private tjn_tools
    package: exact pycountry lookup, then fuzzy search, with a small manual
    map for UN-style names. Unmatched names are printed loudly and return
    None — the row then fails the ISO merge."""
    manual = {
    ...
    try:
        ...
        pass
    print(f"Warning: could not map country name to ISO3: {name!r}")
    return None
...
# %% MARK: 2. Import and clean CbCR data
# 1. Import and clean CbCR data

# Aggregate-row handling (method): WXD residual construction and the
# post-correction WXD consistency fix.
...
# Guard: pivot_table's default aggfunc silently AVERAGES duplicate rows. The
# selected measures must be unique per (parent, partner, grouping, year) cell —
# the raw file already carries duplicate cells on measures we do NOT select
# (ETR percentiles, distinguished only by STATISTICAL_OPERATION), so a future
# vintage adding such a dimension to a selected measure would corrupt the data
# without this check.
_pivot_index_cols = {
    "REF_AREA", "Reference area", "COUNTERPART_AREA", "Counterpart area",
    "Profit grouped by", "TIME_PERIOD",
}
_value_measures = [m for m in cbcr_variables if m not in _pivot_index_cols]
_n_dup_cells = int(
    cbcr_long.loc[cbcr_long["Measure"].isin(_value_measures)]
    .duplicated(
        subset=["REF_AREA", "COUNTERPART_AREA", "Profit grouped by",
                "TIME_PERIOD", "Measure"]
    )
    .sum()
)
if _n_dup_cells:
    raise ValueError(
        f"{_n_dup_cells} duplicate (cell, measure) rows among the selected CbCR "
        "measures — pivot_table would silently average them; inspect the vintage."
    )
...
# Keep only the configured analysis window. A newer raw vintage can carry
# extra years (e.g. 2023); without this filter they would silently enter the
# ±2-year ETR windows and the final files before `n_years` is updated.
_n_outside_window = int((~cbcr["year"].isin(analysis_years)).sum())
if _n_outside_window:
    print(
        f"Dropping {_n_outside_window:,} rows outside the analysis window "
        f"{analysis_years[0]}-{analysis_years[-1]}."
    )
    cbcr = cbcr[cbcr["year"].isin(analysis_years)].copy()
...
# %% MARK: 2.2 Adjust profits for dividend double counting
...
def correct_for_dividend_double_counting(row, year, corrections, tax_havens):
    ...
    # Foreign profits
        ...
        # The 10% haven and 5% aggregate corrections apply to NON-US parents
        # only — García-Bernardo & Janský (2024, §3): "we remove 10% of
        # profits in tax havens for non-US MNCs". US MNCs' dividend double
        # counting is fully handled by the GBJZ (2026) US-specific foreign
        # correction below (§2.5–2.6: foreign double counting removed from
        # haven profits), so stacking the haircuts would double-correct.
        if row["iso_parent"] != "USA":
            taxhaven_correction = (
...
# Country-specific deviations from the GENERAL 35% domestic correction that
# every parent receives for 2016-2019 (applied first, then overridden by the
# entries here). NB the 0.00 entries are NOT no-ops: they switch the default
# 35% OFF for parents whose domestic ETR shows no sign of dividend inflation.
correction_domestic_other_countries = {
    year: {
        # Domestic ETR substantially below foreign ETR -> stronger correction
        "BEL": 0.50,
        "SGP": 0.50,
        "BMU": 0.50,
        # Domestic ETR exceeds foreign ETR -> no correction (overrides the 35%)
        "LVA": 0.00,
        "SVN": 0.00,
        "LUX": 0.00,
    }
...
# Apply half of the tax haven correction. This covers BOTH the "other"
# residual codes (WXD, A_O, …) and the single-letter continent aggregates
# (A/E/F/S) — the rationale is identical for the two families. W is dropped
# in §1.1.
correction_countrygroups = {
    year: {
        key: 0.05
        for key in (other_country_groups | aggregated_country_groups) - {"W"}
    }
    for year in range(2016, 2020)
}
...
def fill_missing_cit_and_corrected_etrs(df):
    ...
    # CIT is a jurisdiction-level attribute, so its fallbacks are PARTNER-level
    # rates only (average first). The pair-specific ETR is deliberately NOT a
    # fallback: it varies across parents, so it could give the same partner-year
    # different `cit` values on different rows.
    cit_backup_cols = [
        "etr_average_corrected",
        "etr_partner_median_corrected",
        "etr_partner_p25_corrected",
...
# %% MARK: 3. Import and clean other variables
# 2. Import and clean other variables

# Manual imputation values (method): hand-curated GDP/population/CIT/wage
# values from data/raw/macro_variables/manual_imputation_values.csv, applied
# with the fill_if_missing / override rules documented there.
def load_manual_imputation_values():
    """Load the curated manual macro / CIT / wage values.

    Single source of truth for the hand-maintained values this script applies
    (small-territory GDP/population, the flat CIT overrides, and the
...
# %% MARK: 3.2 Import and clean corporate income tax rates
...
# a derived (multiplicative) adjustment, so it is applied in code; MTQ<-FRA and
# BVT<-NOR above are likewise derived. The flat and fill-if-missing CIT
# overrides (GIB, MCO, AND, CAF, ..., SMR) live in
...
# %% MARK: 3.3 Import and clean population and GDP data
# 2.3 Import and clean population and GDP data
...
# Only the POPULATION series of this DataBank export is used — GDP comes
# exclusively from the World Bank snapshot merged below (that snapshot
# covers every country-year the export's GDP series covers, so nothing
...
gdp_population = gdp_population[["iso_partner", "year", "population"]].copy()
...
# GDP: one World Bank snapshot (config.canonical_gdp_data, series
# NY.GDP.MKTP.CD) is the single GDP source used across the repo (the
# extractive pipeline reads the same file). Merged AFTER the grid expansion
# so every sample jurisdiction-year receives a row even if absent from the
# population export (e.g. the Bahamas); the manual fills (section 2.3a) cover
# the small territories the World Bank does not publish.
_canon_gdp = pd.read_csv(canonical_gdp_data, comment="#").rename(
    columns={"iso3": "iso_partner"}
)
gdp_population = gdp_population.merge(
    _canon_gdp[["iso_partner", "year", "gdp_current_usd"]],
    on=["iso_partner", "year"],
    how="left",
)
...
# Curated values + their source URLs live in
...
# Final panel year often lacks WB values at download time — fill it from the
# preceding year.
for col in ["gdp_current_usd", "population"]:
    values_prev = gdp_population.loc[
        gdp_population["year"] == last_year - 1, ["iso_partner", col]
    ].rename(columns={col: f"{col}_prev"})
    gdp_population = gdp_population.merge(values_prev, on="iso_partner", how="left")

    mask_last_missing = (gdp_population["year"] == last_year) & (
        gdp_population[col].isna()
    )
    gdp_population.loc[mask_last_missing, col] = gdp_population.loc[
        mask_last_missing, f"{col}_prev"
    ]

    gdp_population = gdp_population.drop(columns=[f"{col}_prev"])
...
# %% MARK: 3.4 Import and clean wage data
...
if "country_name" in wages.columns or "iso_partner" in wages.columns:
    ...
    if outliers.any():
        ...
        # Leave rows with wage_monthly=NaN so the CPI extrapolation in 2.4b
        # treats them as missing and re-imputes from clean local-currency anchors.


# %%
# 2.4b CPI-based extrapolation of ILO observations across missing panel years.
#
# Many countries in the panel (especially lower-middle- and low-income
# countries) have only 1-2 ILO survey
...
# local-currency wage observation(s) from the ILO file, scale that value
...
#                      / exchange_rate[year]
#
# When multiple local-currency observations exist, we anchor on the closest year for
# each target. Countries with no local-currency observation skip this step and fall
...
# Apply the same per-country outlier filter to local-currency observations. For
...
# the proper local-currency scale, get caught by the 1/3-median floor, and dropped.
if not wages_lcy.empty:
    ...
    if lcy_outliers.any():
        print(
            f"Local-currency outlier filter dropped {int(lcy_outliers.sum())} "
...
# %% MARK: 3.5 Import region variables and World Bank…
# 2.5 Import region variables and World Bank income groups
...
# World Bank income groups — read from a PINNED, committed snapshot so the
# cleaned dataset is reproducible offline and across the WB's yearly (July)
# reclassifications. If the snapshot is absent, fetch the API once and WRITE
# it (then commit the file); delete the file to refresh deliberately.
from pathlib import Path as _Path

_wb_income_snapshot = _Path(data_raw) / "country_info" / "wb_income_groups_2026-07.csv"

if _wb_income_snapshot.exists():
    wb_income_base = pd.read_csv(_wb_income_snapshot)
    print(f"Read WB income-group snapshot: {_wb_income_snapshot}")
else:
    ...
    if wb_income_base.empty:
        ...
        wb_income_base.to_csv(_wb_income_snapshot, index=False)
        print(
            f"Saved WB income-group snapshot to {_wb_income_snapshot} — "
            "commit this file so the run is reproducible."
        )
...
# -------------------------------------------------
# 1. Overwrite tax havens consistently
# -------------------------------------------------
# The "tax_haven" income group is the REPRESENTATION haven list from
# config (TAX_HAVENS_REPRESENTATION); its definition is documented there and
# in docs/tax_haven_lists.md. Edit the list in config.py, not here.
tax_haven_codes = sorted(TAX_HAVENS_REPRESENTATION)

wb_income.loc[
    wb_income["iso_partner"].isin(tax_haven_codes),
...
# -------------------------------------------------
manual_income_map = {
    # Not covered by the World Bank classification in all sample years
    "ETH": ("low_income", "LIC"),
    ...
    "REU": ("high_income", "HIC"),
    "WLF": ("high_income", "HIC"),
    # Other territories absent from the World Bank list
    "FLK": ("high_income", "HIC"),
    # Cook Islands: not classified by the World Bank; treated as high income
    "COK": ("high_income", "HIC"),
}
...
# Optional 3-group version plus tax havens
...
# %% MARK: 4. Merge final datasets and save outputs
# 3. Merge final datasets and save outputs

cbcr_main = cbcr_etrs_cits_gdp_wages.merge(
    regions,
    on=["iso_partner"],
    how="left",
)
...
# Current EU membership (EU27): the raw file carries an EU28 flag; the
# United Kingdom is excluded in ALL years — the dummy describes membership
# in today's EU, not historical membership.
cbcr_main["eu27"] = cbcr_main["eu28"]
cbcr_main.loc[cbcr_main["iso_partner"] == "GBR", "eu27"] = 0
cbcr_main.drop(columns=["eu28"], inplace=True)

variables_to_replace = ["ukt", "oecd_oct", "oecd", "eu27"]
cbcr_main[variables_to_replace] = cbcr_main[variables_to_replace].fillna(0)

# %%
# NOTE ON THE HAND-COLLECTED VALUES applied above (GDP / population / wages
# for small territories such as San Marino, the French overseas departments,
# Taiwan, the Pacific islands, the Channel Islands; and the flat CIT
# overrides): none of these numbers are written into this script. Each value
# is stored, together with its source URL and a note, in ONE hand-maintained
# file:
#   data/raw/macro_variables/manual_imputation_values.csv
#     (columns: iso_partner, year, variable, value, mode, source_url, note)
# and summarised for human readers in docs/manual_macro_imputation_sources.md.
# This script only READS that file (load_manual_imputation_values /
# apply_manual_values). To add or revise a value — or its source — edit the
# CSV, not the code. The source URLs stay in the CSV; they are not carried
...
# %% MARK: 4.2 Final checks
# 3.2 Final checks

check_duplicates(cbcr_main_allsubgroupsonly, "final dataset (reporting sample only)")

# Macro coverage guarantee: every REAL partner jurisdiction must carry GDP,
# population and a monthly wage — only the aggregate partner codes (continents
# and residuals, which script 2 disaggregates across real markets) may be
# missing them. A future raw-data update that breaks this coverage should fail
# HERE, loudly, not flow silently into the estimation.
_aggregate_partner_codes = aggregated_country_groups | other_country_groups
_real_rows = cbcr_main_allsubgroupsonly[
    ~cbcr_main_allsubgroupsonly["iso_partner"].isin(_aggregate_partner_codes)
]
for _col in ["gdp_current_usd", "population", "wage_monthly"]:
    _missing = _real_rows.loc[_real_rows[_col].isna(), ["iso_partner", "year"]]
    if not _missing.empty:
        raise AssertionError(
            f"{_col} missing for real partner jurisdictions in the final "
            f"dataset: "
            f"{sorted(_missing['iso_partner'].unique())} — fix the raw data "
            "or add manual values in manual_imputation_values.csv before "
            "rerunning."
        )
print(
    "Macro coverage check passed: GDP, population and wages present for all "
    "real partner jurisdictions."
)
