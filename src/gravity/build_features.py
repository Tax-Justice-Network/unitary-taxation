...
#
# Feature set (trimmed to the García-Bernardo & Janský core — "only use what
# GBJ use", ~118 numeric feature columns):
#   INCLUDED — CEPII gravity (incl. its bundled Comtrade/BACI/IMF trade flows)
#   + World Bank WDI unilateral, both sides (GDP, population, consumption,
#     GFCF/investment, imports/exports, FDI, IP royalties, external debt,
#     education AND health/nurses/physicians — GBJ keep these) + WGI governance
#     PCA (both sides) + IMF CDIS FDI + CPIS portfolio (bilateral, both directions).
#   EXCLUDED (outside the GBJ core) — BIS banking claims (disabled below); the
#     in-repo CIT rates, ILO wages, TJN FSI/CTHI secrecy scores, UNU-WIDER GRD
#     tax/resource revenue, WHO health, Orbis group counts, UNCTAD greenfield FDI.
# NB: the WB block is assembled by globbing wb_*.csv (section 3), so it includes
# every indicator fetch_worldbank.py pulls — health among them.
...
# %%
# 5b. In-repo unilaterals — cit / wage / FSI secrecy — are not part of the
# feature set (not in the GBJ core).
...
import pycountry
try:
    raise RuntimeError("BIS features removed 2026-07-24 (not in the GBJ core)")
