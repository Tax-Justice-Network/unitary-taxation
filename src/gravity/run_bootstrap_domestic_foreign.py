...
#
# Spec: employees_payroll + sales_employees_destmnedds (the headline destination
# sales measure), ETR definition = average corrected, MINIMUM ETR = 15% (etrmax_0_15), tax loss
...
# ── Spec ----------------------------------------------------------------------
# employees_payroll (no sales term) + sales_employees_destmnedds
...
FORMULAS = [f.strip() for f in os.environ.get(
    "BOOT_FORMULAS", "employees_payroll,sales_employees_destmnedds").split(",") if f.strip()]
...
# (tax_haven) overwrite (`wb_income_group_official`).
...
def run_script5(run_dataset, input_file=None):
    ...
    if input_file is not None:
        ...
        # finite etrmax thresholds live only in the archived full-grid copy
    subprocess.run([PY, "archive/5_estimate_profit_shifting_full.py"], cwd=SRC, env=env,
