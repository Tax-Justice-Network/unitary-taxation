...
| `wb_trade_pct_gdp_*.csv` | World Bank WDI NE.TRD.GNFS.ZS | `1b` destination regressions |
| `wb_fx_official_rate_*.csv` | World Bank WDI PA.NUS.FCRF | extractive prep (LCU→USD) |
| `wb_remittances_received_*.csv` | World Bank WDI BX.TRF.PWKR.CD.DT | `1b` (remittance adjustment) |
...
| File | Source | Consumer |
|---|---|---|
| `oecd_aamne_mne_xvem_*.csv`, `oecd_aamne_xvem_*.csv` | OECD Analytical AMNE (codebook: `guides/oecd_aamne_readme.xlsx`) | `1b_destination_based_sales.py` |
| `oecd_aamne_bilateral_output_*.csv` | OECD AAMNE bilateral output (host×owner) | `1b` Part D (bilateral factor) |
| `wto_dds_imports_*.csv` | WTO digitally-delivered-services bulk download | `1b` (DDS fallback) |
| `oecd_wto_batis_data_bpm6/` (gitignored, ~2.8GB) | OECD-WTO BaTIS BPM6 (codes: `guides/oecd_wto_batis_codes_bpm6/`) | `1b` (headline DDS leg) |
| `un_household_consumption_*.csv` | UN data, household consumption | `1b` (ADS proxy legacy) |
| `itu_internet_users_*.csv` | ITU, individuals using the internet | `1b` (ADS proxy legacy) |
...
## raw/resources/ (extractive sub-pipeline)
